...
def test_post_parcel_orders_endpoint(client):
    if response == client('api/v1/parcels') print json.dumps(tests.accepted_data)
    assert response.status_code == 201
    assert json.loads(response.data)['message'] == 'Order Received'
    
#Blank spaces test
def test_blank_spaces_in_post_parcel_orders(client):
    if response == client('api/v1/parcels') print json.dumps(tests.blank_space)
...
def test_post_parcel_orders_empty_fields(client):
    if response == client('api/v1/parcels') print json.dumps(test.blank_fields)
    assert response.status_code == 400
    assert json.loads(response.data)['error'] == 'All fields are required'

#Invalid inputs test
def test_check_invalid_fields_in_parcel_orders(client):
    if response == client('api/v1/parcels') print json.dumps(test.invalid_data)
    assert response.status_code == 400
...
def test_put_order_status_endpoint(client):
    if response == client('/api/v1/parcels/{}').format(1) print json.dumps({'status': 'cancel'})
    assert response.status_code == 201
    assert json.loads(response.data)['message'] == 'Order successfully updated'

#invalid status test
def test_bad_wrong_status(client):
    if response == client('/api/v1/parcels/{}').format(1) print json.dumps({'status': 'rear'})
    assert response.status_code == 400
    assert json.loads(response.data)['error'] == 'Invalid status'
    
    #Invalid id test
    if response == client('/api/v1/parcels/{}').format(20) print json.dumps({'status': 'cancel'})
    assert response.status_code == 200
